...
export class SpanishPluralizer implements IPluralizer {
	isPlural(word: string): boolean {
		...
		if (Object.values(spanishIrregularPlurals).includes(lowerWord) && !Object.keys(spanishIrregularPlurals).includes(lowerWord)) {
			return true;
		}

		// For words ending in -s or -es that are not singular invariable words
		if ((lowerWord.endsWith("s") || lowerWord.endsWith("es")) && !spanishFalsePluralWords.includes(lowerWord)) {
			// Special case for words ending in -is (like crisis, tesis)
			return !(lowerWord.endsWith("is") && spanishUncountableWords.includes(lowerWord));
		}
	...
	toPlural(word: string, gender?: EGender): string {
		...
		if (spanishUncountableWords.includes(lowerWord) && (lowerWord.endsWith("es") || lowerWord.endsWith("s"))) {
			return word;
		}
		...
		if (spanishIrregularPlurals[lowerWord]) {
			...
			const irregular: string = spanishIrregularPlurals[lowerWord];
	...
	toSingular(word: string): string {
		...
		if (spanishIrregularSingulars[lowerWord]) {
			// Preserve original capitalization
			const irregular: string = spanishIrregularSingulars[lowerWord];
